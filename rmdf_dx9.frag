...
vec3 normal(ray r, float d, bool bounce, float i)
{
	...
	if (t == DF_SPHERE) {
		...
		dx = dfSphere((mx[0]*e + p).xyz, params.x) - d;
		dy = dfSphere((mx[1]*e + p).xyz, params.x) - d;
		dz = dfSphere((mx[2]*e + p).xyz, params.x) - d;
	} else if (t == DF_BOX) {
		dx = dfBox((mx[0]*e + p).xyz, params.xyz, params.w) - d;
		dy = dfBox((mx[1]*e + p).xyz, params.xyz, params.w) - d;
		dz = dfBox((mx[2]*e + p).xyz, params.xyz, params.w) - d;
	} else if (t == DF_TORUS) {
		dx = dfTorus((mx[0]*e + p).xyz, params.x, params.y, params.z, params.w) - d;
		dy = dfTorus((mx[1]*e + p).xyz, params.x, params.y, params.z, params.w) - d;
		dz = dfTorus((mx[2]*e + p).xyz, params.x, params.y, params.z, params.w) - d;
	} else if (t == DF_PRISM) {
		dx = dfTriPrism((mx[0]*e + p).xyz, params.x, params.y) - d;
		dy = dfTriPrism((mx[1]*e + p).xyz, params.x, params.y) - d;
		dz = dfTriPrism((mx[2]*e + p).xyz, params.x, params.y) - d;
	} else if (t == DF_RING) {
		dx = dfRing((mx[0]*e + p).xyz, params.x, params.y, params.z, params.w) - d;
		dy = dfRing((mx[1]*e + p).xyz, params.x, params.y, params.z, params.w) - d;
		dz = dfRing((mx[2]*e + p).xyz, params.x, params.y, params.z, params.w) - d;
	}
...
{	
	...
	for (int i=0; i<RAY_STEPS; i++) {
		...
		if (dist < THRESHOLD) {
			...
			r.p -= 4.0*THRESHOLD*diffuse * r.d;

			r.d = reflect(r.d, nml);
			offset(r.d, nml, k, k+10.0*dot(nml, r.d), diffuse);
			r.p += 4.0*THRESHOLD * r.d;

			count++;
			//bounce = true;
			
			if (dot(r.transmit, vec3(8.0)) < 1.0 || count > 2.0+(1.0-diffuse)*4.0) {
