...
vec3 shadeBg(vec3 nml)
{
	...
	vec3 bgLight = normalize(vec3(
        cos(iGlobalTime*0.5)*0.1,
		sin(iGlobalTime*0.1)*0.6 - 0.3,
        -1.0
	));
    ...
    float horizonPow = pow(1.0-abs(skyPow), 3.0)*5.0; //(5.0+centerPow);
	...
	bgCol += sun*(sunD*(1.0-sqrt(scattering))+pow( sp, max(64.0, abs(bgLight.y)*512.0) ));
	bgCol += vec3(0.4,0.2,0.15)*(pow( sp, 8.0) + pow( sp, max(8.0, abs(bgLight.y)*128.0) ));
    bgCol *= mix(vec3(0.7, 0.85, 0.95), vec3(1.0, 0.5, 0.1), scattering);
    bgCol *= 1.0 - clamp(bgLight.y*3.0, 0.0, 0.3);
