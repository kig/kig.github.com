...
vec3 shadeBg(vec3 nml)
{
	...
	vec3 bgColz = vec3(0.5, 0.3, 0.15);
	float bgDiff = dot(nml, vec3(0.0, -1.0, 0.0));
	float sunPow = dot(nml, bgLight);
	float sp = max(sunPow, 0.0);
	vec3 bgCol = vec3(0.0);
	bgCol += max(0.0, bgDiff)*vec3(0.35, 0.80, 1.1)*0.7;
	bgCol += max(0.0, -bgDiff)*vec3(0.2);
	bgCol += vec3(0.8, 0.5, 0.35)*( pow(1.0-abs(bgDiff), 16.0)*(0.2*dot(uv,uv)) );
